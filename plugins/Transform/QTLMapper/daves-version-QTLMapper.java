...
public class QTLMapper implements Plugin {
	...
	dbHost = mView.getProperty("qtlmapper.dbhost", "ensembldb.ensembl.org");
	dbPort = mView.getProperty("qtlmapper.dbport", "3306");
	dbName = mView.getProperty("qtlmapper.dbname", "");
	dbUser = mView.getProperty("qtlmapper.dbuser", "anonymous");
	dbPass = mView.getProperty("qtlmapper.dbpass", "");
	...
	qtlButton.addActionListener(new ActionListener() {
		...
		if( speciesNames == null ) // failed to retrive info, better stop
		{
		    mView.alertMessage( "Unable to retrieve a list of species" );
		    return;
		}
	...
	listDialog.getContentPane().setLayout(new GridBagLayout());
	...
	listDialog.getContentPane().add(hostLabel, gbc);
	gbc.gridx = 1;
	listDialog.getContentPane().add(hostField, gbc);
	gbc.gridx = 0;
	gbc.gridy = 1;
	listDialog.getContentPane().add(portLabel, gbc);
	gbc.gridx = 1;
	listDialog.getContentPane().add(portField, gbc);
	gbc.gridx = 0;
	gbc.gridy = 2;
	listDialog.getContentPane().add(userLabel, gbc);
	gbc.gridx = 1;
	listDialog.getContentPane().add(userField, gbc);
	gbc.gridx = 0;
	gbc.gridy = 3;
	listDialog.getContentPane().add(passLabel, gbc);
	gbc.gridx = 1;
	listDialog.getContentPane().add(passField, gbc);
	...
	gbc.gridy = 4;
	gbc.gridwidth = 2;
	listDialog.getContentPane().add(buttonPanel, gbc);
...
*/
	    ...
	    String url = "jdbc:mysql://"+dbHost+"/"+dbListName + "?user="+dbUser;
...
*/
	...
	latestButton.addActionListener(new ActionListener() {
		...
		for (int i = 0; i < dbNames.size(); i++) {
		    ...
		    if ( ( name.indexOf(species) >= 0 ) && (  name.indexOf("core") >= 0 ) ) {
			dbName = name;
		    }
		}
	        // Find QTL probes
	...
	// Lay out form components
	GridBagConstraints gbc = new GridBagConstraints();
	qtlInfoDialog.getContentPane().setLayout(new GridBagLayout());
	...
	gbc.gridheight = 1;
        gbc.anchor = GridBagConstraints.WEST;
	qtlInfoDialog.getContentPane().add(nameLabel, gbc);
	gbc.gridx = 1;
	gbc.gridwidth = 2;
	qtlInfoDialog.getContentPane().add(nameField, gbc);
	gbc.gridx = 0;
	gbc.gridy = 1;
	gbc.gridwidth = 1;
	qtlInfoDialog.getContentPane().add(speciesLabel, gbc);
	gbc.gridx = 1;
	gbc.gridwidth = 2;
	qtlInfoDialog.getContentPane().add(speciesCombo, gbc);
	gbc.gridx = 0;
	gbc.gridy = 2;
	gbc.gridwidth = 1;
	qtlInfoDialog.getContentPane().add(chrLabel, gbc);
	gbc.gridx = 1;
	gbc.gridwidth = 2;
	qtlInfoDialog.getContentPane().add(chrField, gbc);
	gbc.gridx = 0;
	gbc.gridy = 3;
	gbc.gridwidth = 1;
	qtlInfoDialog.getContentPane().add(startLabel, gbc);
	gbc.gridx = 1;
	qtlInfoDialog.getContentPane().add(startField, gbc);
	gbc.gridx = 2;
	qtlInfoDialog.getContentPane().add(startCombo, gbc);
	gbc.gridx = 0;
	gbc.gridy = 4;
	qtlInfoDialog.getContentPane().add(endLabel, gbc);
	gbc.gridx = 1;
	qtlInfoDialog.getContentPane().add(endField, gbc);
	gbc.gridx = 2;
	qtlInfoDialog.getContentPane().add(endCombo, gbc);
	...
	gbc.gridy = 5;
	gbc.gridwidth = 3;
	qtlInfoDialog.getContentPane().add(buttonPanel, gbc);
	...
	// Lay out components in form
        selectDialog.getContentPane().setLayout(new GridBagLayout());
	GridBagConstraints gbc = new GridBagConstraints();
	...
	gbc.gridheight = 1;
        gbc.anchor = GridBagConstraints.WEST;
	selectDialog.getContentPane().add(nameLabel, gbc);
	gbc.gridx = 1;
        gbc.anchor = GridBagConstraints.CENTER;
	selectDialog.getContentPane().add(nameCombo, gbc);
	...
	gbc.gridy = 1;
	gbc.gridwidth = 2;
	selectDialog.getContentPane().add(buttonPanel, gbc);
	...
	System.out.println("loaded ensjfilepath as " + ensjFilePath);
	    ...
	    System.out.println( "testing using ensjfilepath of " + ensjFilePath);
		...
		System.out.println("saving ensjfilepath as " + ensjFilePath);
                mView.putProperty("ensjfilepath", ensjFilePath);
            }
	
	    // ::DH:: this is definately not a good idea!
	    //
            // Exit loop
            // break;
        }
	...
	System.out.println("loaded mysqlfilepath as " + mysqlFilePath);
		...
		try {
		    ...
		    try {
                        mysqlFilePath = mView.getFile("Location of 'mysql-connector-java-3.1.x-bin.jar'", mysqlFilePath);
                    } catch(UserInputCancelled uic) {
			return found;
		    }
		}
 	    ...
 	    System.out.println( "testing using mysqlfilepath of " + mysqlFilePath);
	    ...
	    {
		found = true;
		System.out.println( "mysql driver class found" );
            }
	    else
	    {
		...
		System.out.println("saving mysqlfilepath as " + mysqlFilePath );
	...
	System.out.println("MySQL class loader:" );
	mysqlCL.describe();

	System.out.println("Ensembl class loader:" );
	ensjCL.describe();
    ...
    /**
     * Clean up and free up resources on exiting plugin
     */
	...
	System.out.println("saving settings: host='" + dbHost + "'" );
