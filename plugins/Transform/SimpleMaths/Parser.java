...
*/
	...
	my_parser.setBinaryOperatorSymbols( new String[] 
	    { 
		"*", "/", "+", "-", ">=", "<=", ">", "<", "!=", "=", "and", "or"
	    } );

	my_parser.setUnaryOperatorSymbols( new String[] 
	    { 
		"-", "!"
	    } );
	
	my_parser.setFunctionSymbols( new String[] 
	    { 
		"min", "max", "mean", "log"
	    } );

	my_parser.setIdentifiers(  new String[] 
	    { 
		"bill *1", "ben-2", "ben-3", "grt #*-14"
	    } );


//	my_parser.setFunctionSymbols( new String[] { "atLeast", "someOf", "anyOf", "noneOf", "allOf", "min", "max", "sin", "cos", "tan" } );
    ...
    /*
    public void setKeywordSymbols( String[] symbols )
    {
	// note that the keywords are sorted by length so the longest one is seen first.
	//
    ...
    */
    ...
    //private String[] keyword_symbols         = { };

    private String[] identifiers;

    private String[] all_symbols; // this is generated automatically from the other symbol lists
    ...
    private final void generateSymbolList()
    {
	...
	//addSymbols( all_symbols_v, keyword_symbols );
	
	all_symbols = (String[]) all_symbols_v.toArray( new String[ all_symbols_v.size() ] );
	
	//java.util.Arrays.sort( symbol_array );
    } 
	...
	//public boolean isKeyword() { return ( type == Keyword ); }
	...
	private int getType( String tok_s )
	{
	    ...
	    /*
	    for(int sym=0; sym < keyword_symbols.length; sym++)
	    {
		if( tok_s.equals( keyword_symbols[ sym ] ) )
		{
		    return Keyword;
		}
	    }
...
//	showThings( things );
    ...
    {
	final String target = (String) things.elementAt( index );
	...
	if( identifiers != null )
	{
	    ...
	    {
		// check for both unquoted and quoted versions of the identifier
		...
		if( pos < 0 )
		{
		    // quoted version not found, test for unquoted version
		...
		if( pos >= 0 )
		{
		    //System.out.println("found: " + test + " in " + target );

		    // found something, replace the string
    ...
    {
	// System.out.println("Parser.split(): removing '" + extract + "' at index " + pos );
	...
	{
	    Token token = new Token( extract, -1 ); // don't actually have the original position, so use -1
	    result.add( token );
	}
	...
	public String toString() { return identifier + "[i]"; }

	public IdentifierTermNode( String i ) { identifier = i; }
    }

    /*
    public class KeywordTermNode extends TermNode
    {
	public String keyword;    

	public String toString() { return keyword + "[k]"; }

	public KeywordTermNode( String i ) { keyword = i; }
    }
    */
...
*/
	...
	while( someTokensRemain( things ) )
	{
	    ...
	    //while( identifyKeywordTermNodes( things ) );

	    if( ++limiter > 8 )
	    {
		if( debug_parse )
		    System.out.println( "Parser: ...limiter value exceeded, nothing more can be extracted");
	...
	if( things.size() == 1 )
	{
		...
		throw new ParseError( "Unexpected '" + things.elementAt( 0 ) + "' (not a term node)" );
	}
	if( things.size() > 1 )
	{
	    // hmmm.. something wasn't parsed correctly
	    throw new ParseError( "Unexpected '" + things.elementAt( 1 ) + "' (too much remains unparsed)" );
	}
	else
	{
	...
	for(int t=0; t < things.size(); t++)
	{
		...
		if( things.elementAt( t ) instanceof Token )
		{
			...
			if( t > 0 )
			{
			    ...
			    {
				// not a token, can't be a bracketed expression
			    }
			}
    ...
    /*
    private boolean identifyKeywordTermNodes( Vector things )
    {
	while( removeRedundantBrackets( things ) );
	
	System.out.println( "Parser.identifyKeywordTermNodes()....");
	...
	for(int t=0; t < things.size(); t++)
	{
		...
		{
		    things.set( t, new KeywordTermNode( token.string ) );

		    anything_found = true;
		    
		    System.out.println( "Parser.identifyKeywordTermNodes() : " +  token.string + " is a keyword");
		}
	    }
	}
