...
public class FilterByNameOrValue implements Plugin, ExprData.Filter, ExprData.ExprDataObserver
{
	...
	/*
	   ...
	   rules:
	       ...
	       Name:  NTsel  
              ...
              Value:  (Values tagged as 'Expression')

                      (Values tagged as 'Error') 

                      (Values tagged as 'Probability') 

		      Meas1 . ExpressionValue
		         Meas1 . SpotAttr1
		         Meas1 . SpotAttr2
			 .....
		         Meas1 . SpotAttrM

		      Meas2 . ExpressionValue
		         Meas2 . SpotAttr1
		         Meas2 . SpotAttr2
			 .....
		         Meas2 . SpotAttrM
	...
	*/
     ...
     
	{
	    ...
	    filter_val_jtf.getDocument().addDocumentListener(new FilterValueChangeListener( filter_val_jtf ));
	...
	*/
	...
	for(int m=0;m<edata.getNumMeasurements();m++)
	{
		...
		filter_val_jtf.getDocument().addDocumentListener( new MeasFilterValueChangeListener( mi, filter_val_jtf ) );
	...
	public MeasFilterValueChangeListener( int which_set, JTextField which_jtf ) 
	{ 
	    super();
	    set = which_set;
	    jtf = which_jtf;
	...
	private void propagate(DocumentEvent e)
	{
	    try
	    {
		setMeasurementFilterValue( set, NumberParser.tokenToDouble( e.getDocument().getText(0, e.getDocument().getLength() ) ) );
		if( jtf != null )
		    jtf.setForeground( Color.black ); 
	    }
	    catch (javax.swing.text.BadLocationException ble)
	    {
		//System.err.println("Value not recognised....\n");
		if( jtf != null )
    ...
    {
	public FilterValueChangeListener( JTextField which_jtf ) 
	{ 
	    super();
	    jtf = which_jtf;
	}
	...
	private void propagate(DocumentEvent e)
	{
	    try
	    {
		setFilterValue( NumberParser.tokenToDouble( e.getDocument().getText(0, e.getDocument().getLength() ) ) );
		if( jtf != null )
		    jtf.setForeground( Color.black ); 
	    }
	    catch (javax.swing.text.BadLocationException ble)
	...
	if(filter_mode > 0)
	{
		...
		{ 
		    ...
		    final double v = edata.eValue(m, spot);
		    
		    
		    switch(filter_op)
		    {
		    case 0: // equals
			if( DoubleCompare.doubleEquals( v, filter_value) )
			    valid_cols++;
			break;
		    case 1: // !=
			if( DoubleCompare.doubleNotEquals( v, filter_value) )
			    valid_cols++;
			break;
		    case 2: // >=
			if( DoubleCompare.doubleGreaterEquals( v, filter_value) )
			    valid_cols++;
			break;
		    case 3: // <=
			if( DoubleCompare.doubleLessEquals( v, filter_value) )
			    valid_cols++;
			break;
		    case 4: // >
			if( DoubleCompare.doubleGreaterThan( v, filter_value) )
			    valid_cols++;
			break;
		    case 5: // <
			if( DoubleCompare.doubleLessThan( v, filter_value) )
			    valid_cols++;
			break;
		    case 6: // <> (range)
			if( DoubleCompare.doubleInRange( v, filter_value ) )
			    valid_cols++;
			break;
		    case 7: // !<> (not in range)
			if( DoubleCompare.doubleNotInRange( v, filter_value ) )
			    valid_cols++;
			break;
		    }
		}
	...
	if(any_measurement_filter_on == true)
	{
		...
		{   
		    final double v = edata.eValue(ms, spot);
		    ...
		    {
		    case 0: // equals
			if( DoubleCompare.doubleEquals( v, measurement_filter_value[ms] ) == false)
			    filter_by_measurement = true;
			break;
		    case 1: // not equals
			if( DoubleCompare.doubleNotEquals( v, measurement_filter_value[ms] ) == false )
			    filter_by_measurement = true;
			break;
		    case 2: // >=
			if( DoubleCompare.doubleGreaterEquals( v, measurement_filter_value[ms]) == false )
			    filter_by_measurement = true;
			break;
		    case 3: // <=
			if( DoubleCompare.doubleLessEquals( v,  measurement_filter_value[ms]) == false )
			    filter_by_measurement = true;
			break;
		    case 4: // >
			if( DoubleCompare.doubleGreaterThan( v, measurement_filter_value[ms] ) == false )
			    filter_by_measurement = true;
			break;
		    case 5: // <
			if( DoubleCompare.doubleLessThan( v, measurement_filter_value[ms] ) == false )
			    filter_by_measurement = true;
			break;
		    case 6: // < > (range)
			if( DoubleCompare.doubleInRange( v, measurement_filter_value[ms] ) == false )
			    filter_by_measurement = true;
			break;
		    case 7: // !<> (not in range)
			if( DoubleCompare.doubleNotInRange( v, measurement_filter_value[ms] ) == false )
			    filter_by_measurement = true;
			break;
		    }
		}
