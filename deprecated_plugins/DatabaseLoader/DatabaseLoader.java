...
public class DatabaseLoader implements Plugin
{
    public final static String plugin_name = "DatabaseLoader v1.1";

    public DatabaseLoader(maxdView mview_)
    {
	mview = mview_;
    }
	...
	mview.putIntProperty("dbload.width", p.getWidth());
	mview.putIntProperty("dbload.height", p.getHeight());

	if(h_split_pane != null)
	{
	    mview.putIntProperty("dbload.hsplit", h_split_pane.getDividerLocation());
	    mview.putIntProperty("dbload.vsplit", v_split_pane.getDividerLocation());
	}

	mview.putBooleanProperty("dbload.load_spot_details",  load_name_details[0]);
	mview.putBooleanProperty("dbload.load_probe_details", load_name_details[1]);
	mview.putBooleanProperty("dbload.load_gene_details",  load_name_details[2]);

	mview.putBooleanProperty("dbload.optimise_for_small_joins",  optimise_for_small_joins);
	...
	JPanel bp = createBrowsePanel();
	frame = new JFrame("Database Loader: '" + dbcon.maxd_props.database_name + "' at '" + dbcon.maxd_props.database_location + "'");
    ...
    { 
	PluginInfo pinf = new PluginInfo("Database Loader", "importer", 
					 "Load Measurements from a 'maxdSQL' database", "", 1, 1, 0);
	return pinf;
    }
